...
//            }
    ...
    public function getCustomersDataById() {
        ...
        if (empty($cusCode)) {
            $this->output
                ...
                ->set_output(json_encode(['status' => 'error', 'message' => 'Customer code is required']));
            return;
        }
        ...
        // Optimized creditinvoicedetails sums
        $credit = $this->db->select("
        IFNULL(SUM(CreditAmount), 0) AS CreditAmount,
        IFNULL(SUM(SettledAmount), 0) AS SettledAmount,
    ...
    ")->from('creditinvoicedetails')
        ...
        $arr['total_credit'] = $credit->CreditAmount;
        $arr['total_payment'] = $credit->SettledAmount;
        $arr['return_payment'] = $credit->returnAmount;

        // Filtered settled amount (excluding type 2)
        $arr['non_type2_settled'] = $this->db->select('IFNULL(SUM(SettledAmount), 0) AS SettledAmount')
            ->from('creditinvoicedetails')
            ->where('CusCode', $cusCode)
            ->where('IsCancel', 0)
            ->where('Type !=', 2)
            ->get()->row()->SettledAmount;

        $arr['over_return_complete_payments'] = $this->db->select('IFNULL(SUM(ReturnAmount), 0) AS ReturnAmount')
            ->from('return_payment')
            ->where('CustomerNo', $cusCode)
            ->where('PaymentType', 3)
            ->where('IsOverReturn', 1)
            ->get()->row()->ReturnAmount;

        $arr['return_payments'] = $this->db->select('IFNULL(SUM(ReturnAmount), 0) AS ReturnAmount')
            ->from('return_payment')
            ->where('CustomerNo', $cusCode)
            ->where('IsComplete', 0)
            ->get()->row()->ReturnAmount;
